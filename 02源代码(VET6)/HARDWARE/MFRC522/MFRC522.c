...
//#define MFRC522_SCLK(x)   PDout(7)=(x)
//#define MFRC522_MOSI(x)   PGout(15)=(x)
...
{ 
	...
	//配置PC8为输入模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_8;//8 号引脚
	...
	//PC11更改成PC9配置输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_9 |GPIO_Pin_7;//9号引脚 	PC11更改成PC7
	...
	//PD6 PD7配置输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_6|GPIO_Pin_7;//6 7号引脚
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_OUT;//输出模式
	GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;//推挽 Push Pull；开漏 Open Drain
	GPIO_InitStructure.GPIO_PuPd=GPIO_PuPd_NOPULL;//不使能上下拉电阻
	GPIO_InitStructure.GPIO_Speed=GPIO_Low_Speed;//低速，功耗低，但是引脚响应时间更长
